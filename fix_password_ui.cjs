...
// Replace mobile password UI
code = code.replace(
    '<div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600 mt-0.5">',
    '<div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-600 mt-0.5"><p>User: <span className="font-mono font-semibold text-slate-800">{u.username}</span></p><p>Pass: <span className="font-mono font-semibold text-slate-800">{(u.password && u.password.startsWith(\'$\'+\'2\')) ? \'Terenkripsi (Reset)\' : (u.password || \'-\')}</span></p>'
);

fs.writeFileSync('src/pages/AdminUsers.tsx', code);
console.log('Fixed passwords UI');
